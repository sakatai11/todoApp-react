...
type TodoProps = {
	...
	clickOption: {
		...
		textEditTodo: (id: string) => void;
		statusEditTodo: (id: string) => void;
		saveTodo: () => void;
	};
	isEditing: boolean;
...
const TodoList = ({
	...
	const { deleteTodo, textEditTodo, saveTodo, statusEditTodo } = clickOption;
					...
					<>
						<Button
							...
							onClick={() => {
								if (todo.id) {
									setModalIsOpen(true);
									textEditTodo(todo.id);
									// statusEditTodo(todo.id);
								}
							}}
						>
							<ModeEditIcon />
